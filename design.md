...
## New Page/Layout Checklist

Before finishing a new page or layout:

- [ ] Read this `design.md`.
- [ ] Match the existing dark premium editorial direction.
- [ ] Use existing CSS custom properties before adding new tokens.
- [ ] Keep sharp corners unless the user explicitly asks for a design-system change.
- [ ] Use local fonts and Material Symbols only.
- [ ] Preserve correct `pt-BR` spelling and accentuation.
- [ ] Verify mobile and desktop layouts.
- [ ] Check long labels, headings, and CTA text for clipping or awkward wrapping.
- [ ] Confirm image/video asset paths exist.
- [ ] Add meaningful image alt text.
- [ ] Respect reduced motion and preserve accessible focus states.
- [ ] Preserve one H1, metadata, canonical URL, social metadata, and schema needs.
- [ ] Run `pnpm run build` for page/UI changes.
- [ ] Run `pnpm run test` too when changing modal, keyboard, focus, timer, or DOM-state logic.
